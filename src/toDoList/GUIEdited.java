...
public class GUIEdited extends JFrame {
...
// end of new code
        ...
        // Just a text label telling the user what the field below is for.
        JLabel subtaskLabel = new JLabel("Add subtask to selected:");
        subtaskLabel.setForeground(textColor);
        subtaskLabel.setAlignmentX(Component.LEFT_ALIGNMENT);

        
        // A text field where the user types the name of the new subtask they want to add.
        JTextField subtaskField = new JTextField();
        subtaskField.setMaximumSize(new Dimension(240, 40));
        subtaskField.setBackground(fieldColor);
        subtaskField.setForeground(textColor);
        subtaskField.setCaretColor(textColor);
        subtaskField.setBorder(BorderFactory.createLineBorder(borderColor));
        subtaskField.setAlignmentX(Component.LEFT_ALIGNMENT);
        ...
        addSubtaskBtn.setForeground(textColor);
        addSubtaskBtn.setFocusPainted(false);
        addSubtaskBtn.setBorderPainted(false);
        addSubtaskBtn.setAlignmentX(Component.LEFT_ALIGNMENT);

        inputPanel.add(Box.createVerticalStrut(18));
        inputPanel.add(subtaskLabel);
        inputPanel.add(Box.createVerticalStrut(6));
        inputPanel.add(subtaskField);
        inputPanel.add(Box.createVerticalStrut(6));
        inputPanel.add(addSubtaskBtn);
        ...
        addSubtaskBtn.addActionListener(e -> {
            ...
            String subName = subtaskField.getText().trim();
            if(!subName.isEmpty()) {
                manager.getTasks().get(selected).addSubtask(new Subtask(subName));
...
// end of new code
    ...
    // MADDIE ADDED - new refresh all method
    ...
    // MADDIE ADDED - new showing subtask info method 
    	/* This pops up a window when you click on a task. Inside that popup:
 
		- Shows all existing subtasks as checkboxes you can check/uncheck
		- Checking/unchecking a subtask calls markSubComplete or markSubIncomplete from your Task class
		- Has a text field at the bottom to type a new subtask name
		- Has an "Add Subtask" button that calls t.addSubtask() and then reopens the dialog so you can see the updated list */
    
    private void showTaskDialog(Task t) {
        JDialog dialog = new JDialog(this, t.name, true);
        dialog.setSize(400, 400);
        dialog.setLayout(new BorderLayout());
        dialog.getContentPane().setBackground(new Color(34,34,59));

        // subtask list panel
        JPanel subtaskPanel = new JPanel();
        subtaskPanel.setLayout(new BoxLayout(subtaskPanel, BoxLayout.Y_AXIS));
        subtaskPanel.setBackground(new Color(34,34,59));

        for(Subtask s : t.subtasks) {
            JCheckBox cb = new JCheckBox(s.getSubName(), s.getStatus());
            cb.setForeground(new Color(242,233,228));
            cb.setBackground(new Color(34,34,59));
            cb.addActionListener(e -> {
                if(cb.isSelected()) {
                    t.markSubComplete(s.getSubName());
                } else {
                    t.markSubIncomplete(s.getSubName());
                }
                refreshAll();
            });
            subtaskPanel.add(cb);
        ...
        // add new subtask at bottom
        JTextField newSubtaskField = new JTextField();
        newSubtaskField.setBackground(new Color(74,78,105));
        newSubtaskField.setForeground(new Color(242,233,228));
        newSubtaskField.setCaretColor(new Color(242,233,228));

        JButton addBtn = new JButton("Add Subtask");
        addBtn.setBackground(new Color(154,140,152));
        addBtn.setForeground(new Color(242,233,228));
        addBtn.setFocusPainted(false);
        addBtn.setBorderPainted(false);

        addBtn.addActionListener(e -> {
            String subName = newSubtaskField.getText().trim();
            if(!subName.isEmpty()) {
                t.addSubtask(new Subtask(subName));
                newSubtaskField.setText("");
                refreshAll();
                dialog.dispose();
                showTaskDialog(t); // reopen with updated list
            }
        });

        JPanel bottomPanel = new JPanel(new BorderLayout());
        bottomPanel.setBackground(new Color(34,34,59));
        bottomPanel.add(newSubtaskField, BorderLayout.CENTER);
        bottomPanel.add(addBtn, BorderLayout.EAST);

        JScrollPane scroll = new JScrollPane(subtaskPanel);
        scroll.getViewport().setBackground(new Color(34,34,59));
        scroll.setBackground(new Color(34,34,59));
    ...
    private void refreshCalendar() {
        ...
        // testing
        System.out.println("Total tasks: " + manager.getTasks().size());
        for(Task t : manager.getTasks()) {
            System.out.println("Task: " + t.name + " | Date: " + t.dueDate);
        }
        ...
        for(int day=1; day<=daysInMonth; day++) {
            ...
            for(Task t : manager.getTasks()) {
                if(t.dueDate != null && t.dueDate.equals(currentDate)) {
                    ...
                    // MADDIE EDITED
//                    og: JButton taskButton = new JButton(t.name);
                    
                    /* Instead of just showing the task name on the calendar button, it now also shows the completion status 
                    next to it using your existing getCompletionDisplay() method. So it shows things like:
                    	Homework 3/3 ✓   or    Study Exam 2/3 */

                    JButton taskButton = new JButton(t.name + " " + t.getCompletionDisplay());
                    
                    
                    
                    if(t.priority.equals(Level.HIGH)) {
                        taskButton.setBackground(new Color(146,104,104));
                    }
                    else if(t.priority.equals(Level.MEDIUM)) {
        ...
        for(int i = 0; i < 7; i++) {
            ...
            for(Task t : sortedTasks) {
                if(t.dueDate != null && t.dueDate.equals(currentDay)) {
                    ...
                    /* setCursor changes the mouse cursor to a hand pointer when you hover over the task name, 
                      signaling to the user it's clickable
                      addMouseListener makes it so when you click the task name in the week view, 
                      it opens the showTaskDialog popup for that task */
                    
                    taskName.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
    ...
    private void refreshDueDateView() {
        ...
        for(Task t : sortedTasks) {
            ...
            /* setCursor changes the mouse cursor to a hand pointer when you hover over the task name, 
             * signaling to the user it's clickable. 
             * addMouseListener makes it so when you click the task name in the due date view, 
             * it opens the showTaskDialog popup for that task. */
