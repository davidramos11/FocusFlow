...
// end of new code
       ...
       selectTaskLabel.setForeground(textColor);
       selectTaskLabel.setAlignmentX(Component.LEFT_ALIGNMENT);


       taskList.setBackground(fieldColor);
       taskList.setForeground(textColor);
       taskList.setSelectionBackground(new Color(154,140,152));
       taskList.setSelectionForeground(Color.WHITE);


       JScrollPane taskScroll = new JScrollPane(taskList);
       taskScroll.setMaximumSize(new Dimension(290, 160));
       taskScroll.setPreferredSize(new Dimension(290, 160));
       ...
       for(int day=1; day<=daysInMonth; day++) {
           ...
           for(Task t : manager.getTasks()) {
               if(t.dueDate != null && t.dueDate.equals(currentDate)) {
                   ...
                   if(t.priority == Level.HIGH) {
                       taskButton.setBackground(new Color(146,104,104));
                   }
                   else if(t.priority == Level.MEDIUM) {
       ...
       for(int i = 0; i < 7; i++) {
           ...
           for (Task t : sortedTasks) {
               if (t.dueDate != null && t.dueDate.equals(currentDay)) {
                   itemCount++;
                   itemCount += t.getSubtasks().size();
               }
           }


           int rowHeight = Math.max(90, 45 + itemCount * 30);


           row.setPreferredSize(new Dimension(1000, rowHeight));
           row.setAlignmentX(Component.LEFT_ALIGNMENT);
           row.setMinimumSize(new Dimension(1000, rowHeight));
           ...
           for(Task t : sortedTasks) {
               if(t.dueDate != null && t.dueDate.equals(currentDay)) {
                   ...
                   for (Subtask s : t.getSubtasks()) {
                       JPanel subRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 1));
                       subRow.setOpaque(false);
                       subRow.setBorder(BorderFactory.createEmptyBorder(0, 58, 0, 0));
                       subRow.setMaximumSize(new Dimension(Integer.MAX_VALUE, 24));


                       JLabel arrow = new JLabel("ᯓ★");
                       arrow.setForeground(new Color(200,190,210));


                       JLabel subCircle = new JLabel(s.getStatus() ? "✓" : "");
                       subCircle.setPreferredSize(new Dimension(17, 17));
                       subCircle.setHorizontalAlignment(SwingConstants.CENTER);
                       subCircle.setOpaque(true);
                       subCircle.setBackground(new Color(58, 61, 90));
                       subCircle.setForeground(new Color(242,233,228));
   ...
   private void refreshDueDateView() {
       ...
       for(Task t : sortedTasks) {
           ...
           for (Subtask s : t.getSubtasks()) {
               JPanel subRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 1));
               subRow.setOpaque(false);
               subRow.setBorder(BorderFactory.createEmptyBorder(0, 95, 0, 0));
               subRow.setMaximumSize(new Dimension(Integer.MAX_VALUE, 24));
